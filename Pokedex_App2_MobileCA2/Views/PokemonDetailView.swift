                            ...
                            ForEach(stats, id: \.stat.name) { pokemonStat in
                                VStack(alignment: .leading) {
                                    Text("\(pokemonStat.stat.name.capitalized):")
                                        .font(.headline)
                                        .foregroundColor(.gray)
                                    ProgressBar(value: Double(pokemonStat.base_stat) / GetHighestStat(pokemonStat.stat.name) )
                                    Text("\(pokemonStat.base_stat) / \(GetHighestStat(pokemonStat.stat.name),specifier: "%.0f")")
                                        .font(.footnote)
                                        .foregroundColor(.gray)
                                }
                            }
...
func GetHighestStat(_ statName: String) -> Double{
    switch statName {
    case "attack":
        return 190
    case "hp":
